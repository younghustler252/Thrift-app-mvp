...
const createPayout = asyncHandler(async (req, res) => {
  ...
  if (!group.readyForPayout) return res.status(400).json({ message: 'Group is not ready for payout yet' });

  const receiverId = group.currentReceiver;
  if (!receiverId) return res.status(400).json({ message: 'No current receiver defined' });
  ...
  if (existing) {
    return res.status(400).json({ message: 'Payout already created for this cycle' });
  }
...
const completePayout = asyncHandler(async (req, res) => {
  ...
  const currentIndex = group.rotationOrder.findIndex(u => u.toString() === payout.receiver.toString());
