...
const makeContribution = asyncHandler(async (req, res) => {
  ...
  if (existing) {
    return res.status(400).json({ error: "You've already submitted a contribution for this cycle" });
  }

  // Validate amount
  if (group.contributionType === "fixed" && amount !== group.contributionAmount) {
    throw new Error(`This group requires a fixed amount of ₦${group.contributionAmount}`);
  }
...
const verifyContribution = asyncHandler(async (req, res) => {
  ...
  if (contribution.status !== "pending") {
    return res.status(400).json({ message: `Already ${contribution.status}` });
  }
  ...
  if (status === "approved") {
    ...
    if (allPaid) {
      ...
      if (group.contributionType === "fixed") {
        ...
        payoutAmount = cycleContributions.reduce((sum, c) => sum + c.amount, 0);
      }
